...
ATPS_GameCharacter::ATPS_GameCharacter() {
...
void ATPS_GameCharacter::SetupPlayerInputComponent(
  ...
  PlayerInputComponent->BindAxis("MoveForward", this,
                                 &ATPS_GameCharacter::MoveForward);
  PlayerInputComponent->BindAxis("MoveRight", this,
                                 &ATPS_GameCharacter::MoveRight);
  ...
  PlayerInputComponent->BindAxis("TurnRate", this,
                                 &ATPS_GameCharacter::TurnAtRate);
  PlayerInputComponent->BindAxis("LookUp", this,
                                 &APawn::AddControllerPitchInput);
  PlayerInputComponent->BindAxis("LookUpRate", this,
                                 &ATPS_GameCharacter::LookUpAtRate);

  // handle touch devices
  PlayerInputComponent->BindTouch(IE_Pressed, this,
                                  &ATPS_GameCharacter::TouchStarted);
  PlayerInputComponent->BindTouch(IE_Released, this,
                                  &ATPS_GameCharacter::TouchStopped);

  // VR headset functionality
  PlayerInputComponent->BindAction("ResetVR", IE_Pressed, this,
                                   &ATPS_GameCharacter::OnResetVR);
}

void ATPS_GameCharacter::OnResetVR() {
  // If TPS_Game is added to a project via 'Add Feature' in the Unreal Editor
  // the dependency on HeadMountedDisplay in TPS_Game.Build.cs is not
...
void ATPS_GameCharacter::TouchStarted(ETouchIndex::Type FingerIndex,
                                      FVector Location) {
  Jump();
}

void ATPS_GameCharacter::TouchStopped(ETouchIndex::Type FingerIndex,
                                      FVector Location) {
  StopJumping();
}

void ATPS_GameCharacter::TurnAtRate(float Rate) {
  // calculate delta for this frame from the rate information
  AddControllerYawInput(Rate * BaseTurnRate * GetWorld()->GetDeltaSeconds());
}

void ATPS_GameCharacter::LookUpAtRate(float Rate) {
  // calculate delta for this frame from the rate information
  AddControllerPitchInput(Rate * BaseLookUpRate *
                          GetWorld()->GetDeltaSeconds());
}

void ATPS_GameCharacter::MoveForward(float Value) {
...
void ATPS_GameCharacter::MoveRight(float Value) {
  if ((Controller != nullptr) && (Value != 0.0f)) {
